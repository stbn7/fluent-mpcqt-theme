...
void DrawnSlider::paintEvent(QPaintEvent *event)
{
    ...
    if (highContrast) {
        ...
        QColor handleBorder1 = "#464240";
        QColor handleFill1 = "#ff9409";
...
void MediaSlider::makeBackground()
{
    ...
    // Draw inside area
    p.setPen(Qt::NoPen);
    QColor grooveFill1 = "#a6a5a8";
    p.setBrush(grooveFill1);
    dr(&p, grooveArea);
...
void MediaSlider::makeHandle()
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        p.setBrush(Qt::NoBrush);
        p.setPen(QPen(handleBorder1, 4, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
        dr(&p, slider);
        p.setPen(QPen(handleFill1, 2, Qt::SolidLine, Qt::SquareCap, Qt::RoundJoin));
        dr(&p, slider);
...
void VolumeSlider::makeBackground()
{
    ...
    backgroundPic = QImage(pw, ph, QImage::Format_RGB32);
    backgroundPic.fill(orange);
    QPainter p(&backgroundPic);
    p.scale(pr, pr);

    double x1 = drawnArea.left() + 0.1;
    double y1 = drawnArea.top() + 0.1;
    double x2 = drawnArea.width() - 0.1;
    double y2 = drawnArea.height() - 0.1;
