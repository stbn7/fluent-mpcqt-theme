...
static const char textWindowTitle[] = "Media Player Classic Qute Teather";
...
void MainWindow::setupVolumeSlider()
{
    ...
    ui->sliderVolumenPrototipe->setRange(0,130);
    ui->sliderVolumenPrototipe->setValue(volumeSlider_->value());
...
void MainWindow::setupStatus()
{
    ...
    ui->durationLayout->insertWidget(0, timePosition);

    timeDuration = new StatusTime();
    ui->statusbarLayout->insertWidget(4, timeDuration);
    ui->durationLayout->insertWidget(2, timeDuration);
...
void MainWindow::setUiEnabledState(bool enabled)
{
    ...
    ui->library->setEnabled(enabled);
    ui->shuffle->setEnabled(enabled);
    ui->fullscreen->setEnabled(enabled);

    ui->stepBackward->hide();
    ui->stepForward->hide();
    ui->loopA->hide();
    ui->loopB->hide();
    ...
    //ui->sliderSeekBarPrototipe->setEnabled(enabled);
}
...
void MainWindow::setTime(double time, double length)
{
    ...
    //ui->sliderSeekBarPrototipe->setMaximum(length >= 0 ? length : 0);
    //ui->sliderSeekBarPrototipe->setValue(time >= 0 ? time : 0);
...
void MainWindow::on_actionPlayVolumeUp_triggered()
{
    ...
    ui->sliderVolumenPrototipe->setValue(newvol);
}
...
void MainWindow::on_actionPlayVolumeMute_toggled(bool checked)
{
    ...
    ui->mute->setChecked(checked);//XXxx
}
...
void MainWindow::position_sliderMoved(int position)
{
    ...
    //ui->sliderSeekBarPrototipe->setValue(position);
}
...
void MainWindow::position_hoverValue(double value, QString text, double x)
{
    ...
    //QPoint where = ui->sliderSeekBarPrototipe->mapToGlobal(QPoint(int(x), timeTooltipAbove ? -40 : 0));
    //QToolTip::showText(where, t, ui->sliderSeekBarPrototipe);
...
void MainWindow::volume_sliderMoved(double position)
{
    ...
    ui->sliderVolumenPrototipe->setValue((int(position)));
}
...
{
    //MainWindow::on_actionPlayAfterAlwaysRepeat_triggered();
    if(ui->shuffle->isChecked())
    {
        MainWindow::on_actionPlayAfterOnceRepeat_triggered();
    }else
    {
        MainWindow::resetPlayAfterOnce();
    }
...
void MainWindow::on_library_clicked()
{
    ...
    {
        playlistWindow_->show();
        ui->library->setChecked(true);
    }
    else
    {
        playlistWindow_->hide();
        ui->library->setChecked(false);
    }
}
...
void MainWindow::on_sliderVolumenPrototipe_valueChanged(int position)
{
...
void MainWindow::on_sliderSeekBarPrototipe_sliderMoved(double value, QString text, double x)
{

    MainWindow::position_hoverValue(value, text, x);
}
